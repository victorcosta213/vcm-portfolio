...
export const onContactCreated = functions
  ...
  .onCreate(async (snap: FirebaseFirestore.QueryDocumentSnapshot) => {
    const data = snap.data() as { name: string; email: string; subject?: string; message: string };

    const subject = (data.subject && data.subject.trim()) || "Novo contato do portfólio";
    const html = `
    ...
    await sgMail.send({ to: MAIL_TO, from: MAIL_FROM, subject, html });
    return null;
  });
